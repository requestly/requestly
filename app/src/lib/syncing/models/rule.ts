...
export class RuleStorageModel extends StorageModel<StorageRecord> {
    ...
    static async createFromSyncModels(
        ...
        const storageEntity = RuleStorageModel.createStorageEntityFromSyncModels(
            ruleDataSyncModel,
    ...
    static async subscribe(callback: (rules: RuleStorageModel[]) => void) {
        ...
        const unsub2 = await RuleMetadataSyncModel.subscribe(
            async (ruleMetadataSyncModels: RuleMetadataSyncModel[]) => {
                // Reduces no of queries on rxdb
                ...
                Promise.allSettled(ruleStorageModelPromises).then(
                    (results: PromiseSettledResult<RuleStorageModel>[]) => {
                        const fulfilledValues = results
                            .filter((result) => result.status === "fulfilled")
                            .map((result) => result.value);
                        const rejectedReasons = results
