...
export function makeAutoGeneratedFieldsSelector(id: string, scopedVariables: ScopedVariables, entry: RQAPI.ApiEntry) {
  const selector = createSelector([makeFilledAuthSelector(id, scopedVariables)], (auth) => {
...
export function useAutoGeneratedFields(entity: HttpRecordEntity | GraphQLRecordEntity) {
  ...
  return useApiClientSelector((state) =>
    makeAutoGeneratedFieldsSelector(id, scopedVariables, entity.getEntityFromState(state).data)(state)
  );
}
