...
import { EntityNotFound, InvalidEntityShape } from "../types";
...
function makeContentTypeSelector(id: string) {
  const selector = createSelector(
    [
      (state: ApiClientStoreState) => selectRecordById(state, id),
    ],
    (record) => {
      if(!record) {
        throw new EntityNotFound(id, "record");
...
export function makeAutoGeneratedFieldsSelector(id: string, scopedVariables: ScopedVariables) {
  const selector = createSelector(
    [makeFilledAuthSelector(id, scopedVariables), makeContentTypeSelector(id)],
...
export function makeUnfilledAutoGeneratedFieldsSelector(id: string) {
  const selector = createSelector(
    [makeAuthResolverSelector(id), makeContentTypeSelector(id)],
    (auth, contentTypeHeader) => {
      const result = extractAuthHeadersAndParams(auth);
      if(contentTypeHeader) {
        result.headers.push({
