...
const parsePrimitiveRecord = (record: Record<string, VariableValueType>): Map<VariableKey, VariableData> => {
  const entries = Object.entries(record).map(([key, value], index) => {
...
export const createDummyVariableStore = (variables: Record<string, string | number | boolean>) => {
  return create<VariablesState>()((set, get) => ({
    data: parsePrimitiveRecord(variables),
