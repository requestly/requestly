...
export function parseContentType(contentType: RequestContentType): NamespaceContents {
  ...
  const contentTypeValue =
    contentType === RequestContentType.FORM ? "multipart/form-data boundary<calculated at runtime>" : contentType;
  headersMap.set("content-type", contentTypeValue);
  return {
    queryParams,
    headers: headersMap,
  };
...
export const createAutogeneratedStore = () => {
  return create<AutogeneratedFieldsStore>()((set, get) => ({
    ...
    purgeAndAddHeaders: (
      ...
      Object.entries(params).forEach(([key, value]) => {
        transformedParams[key] = value.prettyValue || value.value;
      });

      _addHeaders(namespace, transformedParams);
    },
    ...
    /*
    currently we manage the all headers from here
    now for prettyValue we need to seperate out a new function that will only handle the value to be shown
    and function that will be storing the value argument which we will using during the sending the request
    */
    getAllHeaders() {
