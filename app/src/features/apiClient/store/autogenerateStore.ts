...
export interface AutogeneratedFieldsStore {
  namespaces: Partial<Record<Namespace, NamespaceContents>>;
  ...
  _addQueryparams: (namespace: Namespace, params: SimpleKeyValuePair[]) => void;
  purgeAndAddQueryParams: (namespace: Namespace, params: SimpleKeyValuePair[]) => void;

  purgeAndAdd: (namespace: Namespace, headers: Record<string, string>, queryParams: SimpleKeyValuePair[]) => void;
}
...
export const createAutogeneratedStore = (entry?: RQAPI.Entry) => {
  return create<AutogeneratedFieldsStore>()((set, get) => ({
    ...
    _addQueryparams: (namespace: Namespace, params: SimpleKeyValuePair[]) => {
      const { queryParamsVersion, namespaces } = get();

      console.log("DEBUG-y", namespace, params);
      const updatedState = {
        namespaces: {
          ...namespaces,
          [namespace]: {
      ...
      set(updatedState);
      console.log("x", updatedState);
    },

    purgeAndAddQueryParams: (namespace: Namespace, params: SimpleKeyValuePair[]) => {
      const { clearNamespace, _addQueryparams } = get();
      clearNamespace(namespace, true);
      _addQueryparams(namespace, params);
    },

    purgeAndAdd: (namespace: Namespace, headers: Record<string, string>, queryParams: SimpleKeyValuePair[]) => {
