...
import { HeadersType } from "../constants";
...
*/
export const resolveHeaders = (key: string, value: string): HeaderEntry => {
  switch (key) {
    case HeadersType.CONTENT_TYPE: {
      return resolveContentTypeHeaders(value);
    }
  }
};
...
export interface AutogeneratedFieldsStore {
  ...
  getHeadersWithPrettyValue: () => SimpleKeyValuePair[];
...
export function parseContentType(contentType: RequestContentType): NamespaceContents {
  ...
  headersMap.set(HeadersType.CONTENT_TYPE, resolveHeaders(HeadersType.CONTENT_TYPE, contentType));
  return {
    queryParams,
    headers: headersMap,
  };
