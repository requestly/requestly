...
import { extractAuthHeadersAndParams, inheritAuthFromParent } from "../helpers/auth";
...
export interface AutogeneratedFieldsStore {
  ...
  //Header methods
  initialize: <T extends RQAPI.Entry>(
    entry: T,
...
export function parseAuth(
  ...
  console.log("called on save");
  if (auth.currentAuthType === Authorization.Type.INHERIT) {
    console.log("collection-invoked", childDetails, allRecords);
    auth = inheritAuthFromParent(childDetails, allRecords);
  }
  console.log("myNewAuth", auth);
...
function parseRequestEntry(
  ...
  const { auth } = entry;
  const authNamespaceContents = parseAuth(auth, childDetails, allRecords);
...
export const createAutogeneratedStore = () => {
  return create<AutogeneratedFieldsStore>()((set, get) => ({
    ...
    initialize: <T extends RQAPI.Entry>(
      ...
      const namespaces = parseRequestEntry(result, childDetails, allRecords);
      set({
        namespaces,
      });
    },
    ...
    purgeAndAdd: (
      ...
      console.log("purge and add called", headers);
