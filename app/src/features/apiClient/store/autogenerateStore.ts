...
export interface AutogeneratedFieldsStore {
  ...
  //Header methods
  initialize: <T extends RQAPI.Entry>(input: T, resolver: <U extends Record<string, any>>(input: U) => T) => void;
...
export function parseAuth(auth: RQAPI.Auth, resolver?: <U extends Record<string, any>>(input: U) => U): NamespaceContents {
...
export const createAutogeneratedStore = (entry?: RQAPI.Entry) => {
  return create<AutogeneratedFieldsStore>()((set, get) => ({
    ...
    initialize: <T extends RQAPI.Entry>(entry: T, resolver: <U extends Record<string, any>>(input: U) => T) => {
