...
export const MoveToCollectionModal: React.FC<Props> = ({ isOpen, onClose, recordToMove }) => {
  ...
  const moveRecordToCollection = useCallback(
    async (collectionId: string, isNewCollection: boolean) => {
      ...
      const result = await upsertApiRecord(user?.details?.profile?.uid, updatedRequest, activeWorkspaceId);
      if (result.success) {
    ...
    [user?.details?.profile?.uid, activeWorkspaceId, onSaveRecord, recordToMove, navigate]
  );
