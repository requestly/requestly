...
export const getRankForDroppedRecord = ({
  ...
  const siblings = apiRecordsRankingManager.sort(getImmediateChildrenRecords(context, targetRecord.collectionId ?? ""));
  ...
  const rank = apiRecordsRankingManager.getRanksBetweenRecords(beforeRecord, afterRecord, [droppedRecord])[0];
  return rank;
};
