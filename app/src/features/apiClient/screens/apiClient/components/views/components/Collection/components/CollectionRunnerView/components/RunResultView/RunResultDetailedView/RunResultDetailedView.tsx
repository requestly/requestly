...
import { getEventByRecordAndIt } from "store/features/apiclient/selectors";
...
export const RunResultDetailedView: React.FC<Props> = ({ onClose, requestExecutionResult }) => {
  const selectedEvent = useSelector(
    getEventByRecordAndIt(requestExecutionResult?.recordId, requestExecutionResult?.iteration)
  );
  const {res, req} = useMemo(() => {
    let req: RQAPI.Request | undefined = undefined;
    let res: RQAPI.Response | undefined = undefined;
    for(const event of selectedEvent){
        if(event.data.type === "REQUEST"){
            req = event.data.payload;
        }
    ...
    return [
      {
        key: "response",
        label: "Response",
        children: <ResponseTab response={res} />,
      },
      {
        key: "request",
        label: "Request",
        children: <RequestTab request={req} />,
      },
    ];
