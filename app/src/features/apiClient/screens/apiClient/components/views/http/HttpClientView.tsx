...
const HttpClientView: React.FC<Props> = ({
  ...
  const [canGenerateTests, setCanGenerateTests] = useState(false);
  ...
  const handleGenerateTests = useCallback(async () => {
    ...
    try {
      ...
      if (hasAutogeneratedTests(existingScript)) {
      ...
      setEntry((prev) => ({
        ...prev,
        scripts: { ...prev.scripts, postResponse: newScript },
      }));

      const rerunResult = await httpRequestExecutor.rerun(apiEntryDetails?.id, {
        ...entry,
        scripts: { ...entry.scripts, postResponse: newScript },
      });
      if (rerunResult.status === RQAPI.ExecutionStatus.SUCCESS) {
        setEntry((prev) => ({
          ...prev,
          testResults: rerunResult.artifacts.testResults,
        }));
      } else {
        toast.error("Failed to execute autogenerated tests");
      }
    } catch (e) {
  ...
  }, [entry, isGeneratingTests, httpRequestExecutor, apiEntryDetails?.id, setEntry]);

  useEffect(() => {
    const postResponseScript = entry.scripts?.postResponse || "";
    const scriptWithoutAutogen = stripAutogeneratedTests(postResponseScript);
    const hasManualTests = /(?:rq|RQ)\.test\(/.test(scriptWithoutAutogen);
    const alreadyAutogenerated = hasAutogeneratedTests(postResponseScript);
    const responseExists = Boolean(entry.response);
    const hasTestResults = Boolean(entry.testResults?.length);

    const eligible = responseExists && !hasManualTests && !alreadyAutogenerated && !hasTestResults;

    setCanGenerateTests(eligible);
  }, [entry.response, entry.testResults, entry.scripts?.postResponse]);
