...
const HttpClientView: React.FC<Props> = ({
  ...
  const httpRequestExecutor = useHttpRequestExecutor(apiEntryDetails.collectionId ?? undefined);
  ...
  const handleGenerateTests = useCallback(async () => {
    ...
    try {
      ...
      setEntry((prev) => ({
        ...prev,
        scripts: { postResponse: newScript, preRequest: prev.scripts?.preRequest || "" },
      }));
    ...
    try {
      ...
      if (apiClientExecutionResult.status === "success") {
        ...
        trackResponseLoaded({
          ...
          time: Math.round((executedEntry.response?.time ?? 0) / 1000),
        });
        trackApiRequestDone({
  ...
  const handleRecordNameUpdate = async () => {
    ...
    if (result.success && result.data.type === RQAPI.RecordType.API) {
      ...
      const savedRecord: RQAPI.HttpApiRecord = {
        ...(apiEntryDetails ?? {}),
        ...result.data,
        data: { ...(result.data.data as RQAPI.HttpApiEntry), ...record.data },
      };
  ...
  const onSaveButtonClick = useCallback(async () => {
    ...
    const result = isCreateMode
      ? await apiClientRecordsRepository.createRecordWithId(record, record.id!) // fix-me: forced
      : await apiClientRecordsRepository.updateRecord(record, record.id!);
