...
import { KeyValueTableBulkEditPanel } from "../components/request/components/KeyValueTable/KeyValueTableBulkEditPanel/KeyValueTableBulkEditPanel";
...
const HttpClientView: React.FC<Props> = ({
  ...
  const [bulkEditTableType, setBulkEditTableType] = useState<"headers" | "queryParams" | null>(null);
  const [showHeadersDescription, setShowHeadersDescription] = useState(false);
  const [showQueryParamsDescription, setShowQueryParamsDescription] = useState(false);
  ...
  return isExtensionEnabled ? (
    <div className="api-client-view http-client-view">
      ...
      >
        {isBulkEditPanelOpen && bulkEditTableType ? (
          <div
            className={`api-client-body-with-bulk-edit ${
              sheetPlacement === BottomSheetPlacement.BOTTOM ? "horizontal-split" : "vertical-split"
            }`}
          >
            <div className="api-client-table-pane">
              <HttpRequestTabs
                error={error}
                requestEntry={entry}
                ...
                bulkEditConfigs={{
                  ...
                  showHeadersDescription,
                  onShowHeadersDescriptionChange: setShowHeadersDescription,
                  showQueryParamsDescription,
                  onShowQueryParamsDescriptionChange: setShowQueryParamsDescription,
                }}
              />
            </div>
            <div className="api-client-bulk-edit-pane">
              <KeyValueTableBulkEditPanel
                data={bulkEditTableType === "headers" ? entry.request.headers : queryParams}
                onClose={() => setIsBulkEditPanelOpen(false)}
                onChange={(updatedPairs) => {
                  if (bulkEditTableType === "headers") {
                    setRequestEntry((prev) => ({
                      ...prev,
                      request: { ...prev.request, headers: updatedPairs },
                    }));
                  } else if (bulkEditTableType === "queryParams") {
          ...
          <div
            className={`api-client-body ${
              sheetPlacement === BottomSheetPlacement.BOTTOM
                ? "api-client-body__vertical"
                : "api-client-body__horizontal"
            }`}
          >
            <HttpRequestTabs
              error={error}
