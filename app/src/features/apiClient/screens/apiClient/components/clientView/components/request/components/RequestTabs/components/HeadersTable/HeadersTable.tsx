...
export const HeadersTable: React.FC<HeadersTableProps> = ({ headers, variables, setRequestEntry }) => {
  const [headerVersion, getHeadersToShow] = useAutogenerateStore((s) => [s.headerVersion, s.getHeadersToShow]);

  const autogeneratedHeaders = useMemo(() => getHeadersToShow(), [headerVersion, getHeadersToShow]);
