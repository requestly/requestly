...
export const HeadersTable: React.FC<HeadersTableProps> = ({ headers, variables, setRequestEntry }) => {
  const [headerVersion, getHeaderWithPrettyValue] = useAutogenerateStore((s) => [
    s.headerVersion,
    s.getHeaderWithPrettyValue,
  ]);

  const autogeneratedHeaders = useMemo(() => getHeaderWithPrettyValue(), [headerVersion, getHeaderWithPrettyValue]);
