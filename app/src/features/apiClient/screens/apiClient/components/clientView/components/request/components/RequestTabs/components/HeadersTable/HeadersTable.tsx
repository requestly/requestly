...
export const HeadersTable: React.FC<HeadersTableProps> = ({ headers, variables, setRequestEntry }) => {
  const [headerVersion, getHeadersWithPrettyValue] = useAutogenerateStore((s) => [
    s.headerVersion,
    s.getHeadersWithPrettyValue,
  ]);

  const autogeneratedHeaders = useMemo(() => getHeadersWithPrettyValue(), [headerVersion, getHeadersWithPrettyValue]);
