...
const APIClientView: React.FC<Props> = ({
  ...
  const apiClientExecutorRef = useRef<ApiClientExecutor | null>(null);
  ...
  useEffect(() => {
    apiClientExecutorRef.current = new ApiClientExecutor(
      apiEntryDetails.id,
      ctx,
      appMode,
  ...
  }, [apiClientWorkloadManager, apiEntryDetails.id, appMode, autoGeneratedStore, ctx]);
  ...
  useEffect(() => {
    ...
    const namespaces = parseRequestEntry(resolvedEntry, childDetails, getParentChain, getData);
  ...
  const onSendButtonClick = useCallback(async () => {
    ...
    apiClientExecutorRef.current?.updateEntryDetails({
  ...
  const handleCancelRequest = useCallback(() => {
    apiClientExecutorRef.current?.abort();
    trackAPIRequestCancelled();
  ...
  const handleTestResultRefresh = useCallback(async () => {
    try {
      ...
      const result = await apiClientExecutorRef.current?.rerun();
  ...
  }, [apiClientExecutorRef, apiEntryDetails?.id, apiEntryDetails?.collectionId, entry]);
  ...
  useEffect(() => {
    if (apiClientExecutorRef.current) {
      apiClientExecutorRef.current.updateInternalFunctions({
    ...
    <div className="api-client-view">
      ...
      {isSnippetModalVisible ? (
        <ApiClientSnippetModal
          apiRequest={apiClientExecutorRef.current?.prepareRequest()}
