import React, { useCallback, useContext, useEffect, useLayoutEffect, useMemo, useState, useRef } from "react";
...
const APIClientView: React.FC<Props> = ({
  ...
  const changedCollection = useMemo(() => {
  ...
  useEffect(() => {
    ...
    autoGeneratedStore.getState().initialize(entry, childDetails, apiClientRecords, resolver);
  }, []);

  //this will run when collection auth is changed
  useEffect(() => {
    ...
    const { headers, queryParams } = parseAuth(apiEntryDetails.data.auth, childDetails, apiClientRecords, resolver);
  ...
  const handleAuthChange = useCallback(
    (newAuth: RQAPI.Auth) => {
      ...
      const { headers, queryParams } = parseAuth(newAuth, childDetails, apiClientRecords, resolver);
      const headersContent = Object.fromEntries(headers);
    ...
    [apiClientRecords, apiEntryDetails, purgeAndAdd, resolver]
  );
