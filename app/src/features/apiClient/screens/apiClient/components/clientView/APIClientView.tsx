...
import { AutogeneratedFieldsNamespace, SimpleKeyValuePair } from "features/apiClient/store/autogenerateStore";
...
const APIClientView: React.FC<Props> = ({
  ...
  useEffect(() => {
    // On mounting only, we are intializing, so dep array is empty
    autoGeneratedStore.getState().initialize(entry, resolver);
  }, []);
  ...
  const handleAuthChange = useCallback(
    (newAuth: RQAPI.Auth) => {
      const resolvedAuth = resolver(newAuth);
      const { headers, queryParams } = extractAuthHeadersAndParams(resolvedAuth);
