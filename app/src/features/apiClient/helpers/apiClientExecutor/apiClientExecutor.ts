...
export class ApiClientExecutor {
  ...
  prepareRequest() {
    ...
    const { getHeadersOnPrepareRequest, getAllQueryParams } = this.autoGeneratedStore.getState();
    const headers = getHeadersOnPrepareRequest().map((kv, index) => ({ ...kv, id: index, isEnabled: true }));
