...
export class ApiClientExecutor {
  ...
  prepareRequest() {
    ...
    const { getHeaderWithOriginalValue, getAllQueryParams } = this.autoGeneratedStore.getState();
    const headers = getHeaderWithOriginalValue().map((kv, index) => ({ ...kv, id: index, isEnabled: true }));
