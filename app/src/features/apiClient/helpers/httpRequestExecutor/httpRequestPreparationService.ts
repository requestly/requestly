...
import { makeAutoGeneratedFieldsSelector, makeUnfilledAutoGeneratedFieldsSelector } from "features/apiClient/slices/autoGeneratedFields/selectors";
...
export class HttpRequestPreparationService {
  ...
  private getVariables(recordId: string, config?: {
    storeOverrideConfig?: StoreOverrideConfig,
    scopes?: Scope[],
  }) {
    const scopedVariables = getScopedVariables(this.ctx.store.getState(), reduxStore.getState().runtimeVariables.entity.variables, recordId, config);
  ...
  private extractVariablesFromContext(
    ...
    // const scopedVariables = getScopedVariables(this.ctx.store.getState(), reduxStore.getState().runtimeVariables.entity.variables, recordId, {storeOverrideConfig});
...
private renderVariables(
  ...
  prepareRequest(
    ...
    const workingEntry = cloneDeep(sanitizeEntry(entity.getEntityFromState(this.ctx.store.getState()).data));
    ...
    const selector = makeAutoGeneratedFieldsSelector(recordId,scopedVariables);
    const {headers: autoGeneratedHeaders, queryParams: autoGeneratedQueryParams} = selector(this.ctx.store.getState());
    ...
    workingEntry.request.queryParams = updateRequestWithAuthOptions(workingEntry.request.queryParams, autoGeneratedQueryParams);
