...
export class FirebaseApiClientRecordsSync implements ApiClientRecordsInterface<ApiClientCloudMeta> {
  ...
  async setCollectionVariables(id: string, variables: EnvironmentVariables): Promise<{ success: boolean; data: RQAPI.CollectionRecord | null; message?: string; }> {
		return this.updateRecord({ data: { variables } }, id) as Promise<{ success: boolean; data: RQAPI.CollectionRecord | null; message?: string; }>;
  }
}
