...
export const tabsSlice = createSlice({
  ...
  reducers: {
    ...
    reorderTabs(state, action: PayloadAction<{ draggedTabId: TabId; targetTabId: TabId }>) {
      ...
      const [draggedId] = tabIds.splice(draggedIndex, 1);

      if (!draggedId) {
        return;
      }

      tabIds.splice(targetIndex, 0, draggedId);
    },
