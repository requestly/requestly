...
export const SplitPaneLayout: React.FC<Props> = ({ bottomSheet, children, minSize = 100, defaultSizing = true }) => {
  ...
  useEffect(() => {
    if (isSheetPlacedAtBottom && splitPane.current) {
      if (isBottomSheetOpen) {
        splitPane.current.split.setSizes(defaultSizing ? [40, 60] : [50, 50]);
      } else {
  ...
  return (
    <div className={`split-pane-layout-wrapper ${defaultSizing ? "default" : ""}`}>
      <Split
        ref={splitPane}
