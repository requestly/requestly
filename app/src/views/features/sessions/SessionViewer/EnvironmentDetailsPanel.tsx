...
const EnvironmentDetailsPanel: React.FC<{ environment: Environment }> = ({
  environment,
}) => {
  return environment ? (
    <div className="session-panel-content placeholder">
      ...
      >
        ...
        <Descriptions.Item label="Operating system" span={3}>
          ...
          {environment.os.name === "macOS" &&
          environment.os.version === "10.15.7"
            ? ` (or above)`
            : null}
        </Descriptions.Item>
        <Descriptions.Item label="Platform" span={3}>
          {aggregate(
            environment.platform.vendor,
            environment.platform.type,
            environment.platform.model
          )}
        </Descriptions.Item>
        <Descriptions.Item label="Screen dimensions" span={3}>
          {environment.screenDimensions.width} x{" "}
          {environment.screenDimensions.height}
        </Descriptions.Item>
        <Descriptions.Item label="Browser dimensions" span={3}>
          {environment.browserDimensions.width} x{" "}
          {environment.browserDimensions.height}
        </Descriptions.Item>
