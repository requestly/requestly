...
BG.Methods.overrideResponse = function (details) {
  ...
  if (responseRules.length > 0) {
    ...
    chrome.tabs.sendMessage(
      details.tabId,
      {
        action: RQ.Constants.CLIENT_MESSAGES.OVERRIDE_RESPONSE,
        url: details.url,
...
BG.Methods.handleExtensionDisabled = function () {
  ...
  chrome.tabs.query({}, (tabs) => {
    tabs.forEach((tab) => {
      ...
      chrome.tabs.sendMessage(tab.id, { action: RQ.Constants.CLIENT_MESSAGES.STOP_RECORDING }, { frameId: 0 });
    });
  });
...
BG.Methods.addListenerForExtensionMessages = function () {
  chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    switch (message.action) {
      case RQ.Constants.CLIENT_MESSAGES.ADD_EVENT:
        EventActions.queueEventToWrite(message.payload);
        break;

      case RQ.Constants.CLIENT_MESSAGES.GET_SCRIPT_RULES:
        if (message.url) {
          sendResponse(BG.Methods.getMatchingRules(message.url, RQ.RULE_TYPES.SCRIPT));
        }
        break;

      case RQ.Constants.CLIENT_MESSAGES.GET_USER_AGENT_RULE_PAIRS:
        if (message.url) {
          sendResponse(BG.Methods.getMatchingRulePairs(message.url, RQ.RULE_TYPES.USERAGENT));
        }
        break;

      case RQ.Constants.CLIENT_MESSAGES.NOTIFY_RULES_APPLIED:
        if (message.rules) {
      ...
      case RQ.Constants.CLIENT_MESSAGES.GET_SESSION_RECORDING_CONFIG:
        BG.Methods.getSessionRecordingConfig(sender.tab.url).then(sendResponse);
        return true;

      case RQ.Constants.CLIENT_MESSAGES.NOTIFY_SESSION_RECORDING_STARTED:
        BG.Methods.onSessionRecordingStartedNotification(sender.tab.id);
        break;

      case RQ.Constants.CLIENT_MESSAGES.NOTIFY_SESSION_RECORDING_STOPPED:
      ...
      case RQ.Constants.CLIENT_MESSAGES.NOTIFY_CONTENT_SCRIPT_LOADED:
...
BG.Methods.onContentScriptLoadedNotification = async (tabId) => {
  ...
  if (cachedAppliesRules?.length > 0) {
    chrome.tabs.sendMessage(
      tabId,
      {
        action: RQ.Constants.CLIENT_MESSAGES.SYNC_APPLIED_RULES,
...
BG.Methods.getExecutedRules = async (tabId, callback) => {
  chrome.tabs.sendMessage(
    tabId,
    {
      action: RQ.Constants.CLIENT_MESSAGES.GET_APPLIED_RULE_IDS,
    },
...
BG.Methods.sendLogToConsoleLogger = async function (rule, requestDetails, modification) {
  ...
  chrome.tabs.sendMessage(
    ...
    {
      action: RQ.Constants.CLIENT_MESSAGES.PRINT_CONSOLE_LOGS,
      requestDetails,
      rule,
      modification,
...
BG.Methods.getTabSession = (tabId, callback) => {
  chrome.tabs.sendMessage(tabId, { action: RQ.EXTENSION_MESSAGES.GET_TAB_SESSION }, { frameId: 0 }, callback);
};
...
BG.Methods.sendAppliedRuleDetailsToClient = async (rule, requestDetails) => {
  ...
  chrome.tabs.sendMessage(tabId, {
    action: RQ.Constants.CLIENT_MESSAGES.UPDATE_APPLIED_RULE_ID,
    ruleId: rule.id,
  });
