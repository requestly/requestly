describe("PreDefined Functions: ", function () {
  describe("rq_rand(N) ", function () {
    ...
    it("should replace multiple rand usages with random values", function () {
      expect(/[0-9]{1}_[0-9]{1}/.test(F.eval("rq_rand(1)_rq_rand(1)"))).toBe(
        true
      );
      expect(/[0-9]{4}_[0-9]{5}/.test(F.eval("rq_rand(4)_rq_rand(5)"))).toBe(
        true
      );
      expect(
        /[0-9]{8}_[0-9]{8}/.test(F.eval("rq_rand(100)_rq_rand(1000)"))
      ).toBe(true);
    });
  });
  ...
  describe("rq_encode(str)", function () {
    ...
    it("should replace rq_encode usages with encoded value", function () {
      expect(F.eval("rq_encode(user+test@example.com)")).toBe(
      ...
      expect(
        F.eval(
          "http://www.example.com/?password=rq_encode(P@$$W0rD)&val=rq_rand(4)#rq_encode(my#string)"
        )
      ).toBe(
  ...
  describe("rq_decode(str)", function () {
    ...
    it("should replace rq_decode usages with encoded value", function () {
      expect(F.eval("rq_decode(user%2Btest%40example.com)")).toBe(
        "user+test@example.com"
      );

      expect(
        F.eval(
          "http://www.example.com/rq_decode(%3Fquery%3Dnumber%26val%3D123%23hash)"
        )
      ).toBe("http://www.example.com/?query=number&val=123#hash");

      expect(
        F.eval(
          "http://www.example.com/rq_decode(%3Fquery%3Dnumber)&val=rq_rand(3)rq_decode(%23hash)"
        )
      ).toBe("http://www.example.com/?query=number&val=rq_rand(3)#hash");
    });
  });
  ...
  describe("rq_increment(N,k)", function () {
    ...
    it("should replace rq_increment usages with value incremented by 1", function () {
      ...
      expect(F.eval("http://www.example.com/?page=rq_increment(10)")).toBe(
    ...
    it("should replace rq_increment usages with value incremented by k", function () {
      ...
      expect(F.eval("http://www.example.com/?page=rq_increment(10,3)")).toBe(
        "http://www.example.com/?page=13"
      );
    });
  ...
  describe("rq_decrement(N,k)", function () {
    ...
    it("should replace rq_decrement usages with value decremented by 1", function () {
      ...
      expect(F.eval("http://www.example.com/?page=rq_decrement(10)")).toBe(
        "http://www.example.com/?page=9"
      );
    });

    it("should replace rq_decrement usages with value decremented by k", function () {
      expect(F.eval("rq_decrement(10,3)")).toBe("7");
      expect(F.eval("http://www.example.com/?page=rq_decrement(10,3)")).toBe(
        "http://www.example.com/?page=7"
      );
    });
