describe("RuleMatcher: ", function () {
  ...
  describe("#matchUrlWithRuleSource", function () {
    ...
    it("should match Redirect Rule Source", function () {
      ...
      expect(
        RuleMatcher.matchUrlWithRuleSource(pair.source, URL_SOURCES.GOOGLE, requestDetails.tabId, pair.destination)
      ...
      expect(RuleMatcher.matchUrlWithRulePairs(redirectRule.pairs, "https://cricket.yahoo.com", requestDetails)).toBe(
    ...
    it("should match different Url components", function () {
      ...
      // Host Matching
      expect(
        RuleMatcher.matchUrlWithRuleSource(pair.source, URL_SOURCES.DROPBOX, requestDetails.tabId, pair.destination)
      ).toBe(URL_SOURCES.FACEBOOK);
    ...
    it("should return null when Cancel Rule Source does not match with Url", function () {
      ...
      expect(RuleMatcher.matchUrlWithRuleSource(pairs[0].source, URL_SOURCES.GOOGLE, requestDetails.tabId)).toBeNull();
      expect(
        RuleMatcher.matchUrlWithRuleSource(pairs[1].source, URL_SOURCES.FACEBOOK, requestDetails.tabId)
      ).not.toBeNull();
    });
    ...
    it("should not match url with black list domains", function () {
      ...
      expect(
        RuleMatcher.matchUrlWithRuleSource(pairs[0].source, "http://blog.requestly.in", requestDetails.tabId)
      ).toBeNull();
      expect(
        RuleMatcher.matchUrlWithRuleSource(pairs[0].source, "http://web.requestly.in", requestDetails.tabId)
      ).toBeNull();
      expect(
        RuleMatcher.matchUrlWithRuleSource(
          pairs[0].source,
  ...
  describe("#matchUrlWithPageSource", function () {
    ...
    it("should match different Url components", function () {
      ...
      // Host Matching
      expect(RuleMatcher.matchUrlWithPageSource(pageSource, URL_SOURCES.DROPBOX)).toBe("");

      expect(RuleMatcher.matchUrlWithPageSource(pageSource, URL_SOURCES.EXAMPLE + "?ref=dropbox")).toBe(null);

      // Path Matching
      pageSource.key = RQ.RULE_KEYS.PATH;
      expect(RuleMatcher.matchUrlWithPageSource(pageSource, URL_SOURCES.DROPBOX)).toBe(null);

      expect(RuleMatcher.matchUrlWithPageSource(pageSource, URL_SOURCES.EXAMPLE + "/dropbox/home.html")).toBe("");
    });

    it("should return null when Page Source does not match with Url", function () {
      expect(RuleMatcher.matchUrlWithPageSource(pageSources[0], URL_SOURCES.GOOGLE)).not.toBeNull();

      expect(RuleMatcher.matchUrlWithPageSource(pageSources[1], URL_SOURCES.FACEBOOK)).toBeNull();

      expect(RuleMatcher.matchUrlWithPageSource(pageSources[2], `example.${KEYWORDS.YAHOO}.com`)).toBeNull();

      expect(
        RuleMatcher.matchUrlWithPageSource(pageSources[2], `https://example.${KEYWORDS.YAHOO}.com`)
      ).not.toBeNull();
    });
  });
  ...
  describe("#checkRegexMatch", function () {
    it("should return null when inputString does not match regex", function () {
      expect(RuleMatcher.checkRegexMatch("/(.+).yahoo.com", "https://google.com", null)).toBeNull();
    });

    it("should return null when regexString is not valid Regex", function () {
      expect(RuleMatcher.checkRegexMatch("invalid/regex", "/path", "")).toBeNull();
    });

    it("should return the final string as is when there are no groups in regex", function () {
      var d = "dest";
      expect(RuleMatcher.checkRegexMatch("/yahoo/ig", "http://cricket.yahoo.com", d)).toBe(d);
    });

    it("should replace group values in final string when there are groups in regex", function () {
      expect(RuleMatcher.checkRegexMatch("/(.+).yahoo.com?q=(.+)", "cricket.yahoo.com?q=rocks", "$1-$2")).toBeNull(
        "cricket-rocks"
      );
      expect(RuleMatcher.checkRegexMatch("/(.+).yahoo.com?q=(.+)", "mail.yahoo.com?q=sucks", "$1-$2")).toBeNull(
        "mail-sucks"
      );
    });

    it("should convert extra/unmatched group values to empty string in final string", function () {
      expect(RuleMatcher.checkRegexMatch("/(.+).yahoo.com?q=(.+)", "cricket.yahoo.com?q=rocks", "$1-$2$3.")).toBeNull(
        "cricket-rocks."
      );
      expect(RuleMatcher.checkRegexMatch("/(.+).yahoo.com?q=(.+)", "mail.yahoo.com?q=sucks", "$1-$2$3.")).toBeNull(
        "mail-sucks."
      );
    });
  });
  ...
  describe("#checkWildCardMatch", function () {
    it("should return null when wildcard expression does not match", function () {
      expect(RuleMatcher.checkWildCardMatch("http://exam*.com", "https://example.com", "$1")).toBeNull();
      expect(RuleMatcher.checkWildCardMatch("http://*exam*.com", "http://exercise.com", "$2-$1")).toBeNull();
    });

    it("should replace $ values as empty string when empty values satisfy the input", function () {
      expect(
        RuleMatcher.checkWildCardMatch("http://*example*.com", "http://example.com", "http://$1example$2.com")
      ).toBe("http://example.com");
    });

    it("should replace $ values in final string when there is match", function () {
      expect(RuleMatcher.checkWildCardMatch("http://*exam*.com", "http://example.com", "$2-$1")).toBe("ple-");
      expect(RuleMatcher.checkWildCardMatch("*://exam*.com", "http://example.com", "$1_$2")).toBe("http_ple");
      expect(RuleMatcher.checkWildCardMatch("*://example.*", "http://example.com", "$1_$2")).toBe("http_com");
      expect(RuleMatcher.checkWildCardMatch("http://*.*", "http://cricket.yahoo.com", "$1_$2")).toBe(
        "cricket_yahoo.com"
      );
      expect(RuleMatcher.checkWildCardMatch("http://*.*.*", "http://cricket.yahoo.com", "$1_$2_$3")).toBe(
        "cricket_yahoo_com"
      );
      expect(
  ...
  describe("#matchUrlWithRulePairs", function () {
    it("should execute multiple pairs of a rule", function () {
      ...
      expect(RuleMatcher.matchUrlWithRulePairs(r.pairs, URL_SOURCES.GOOGLE, requestDetails)).toBe(URL_SOURCES.FACEBOOK);
    });
  });
  ...
  describe("match source filters", function () {
    ...
    describe("#matchPageUrlFilter", function () {
      ...
      it("should return true if page url filter is satisfied", function () {
        const pageUrlFilter = sourceFilters[RQ.RULE_SOURCE_FILTER_TYPES.PAGE_URL][0];

        spyOn(window.tabService, "getTabUrl").andReturn(URL_SOURCES.EXAMPLE);
        expect(RuleMatcher.matchPageUrlFilter(pageUrlFilter, requestDetails)).toBeTruthy();
      });
      ...
      it("should return false if page url filter is not satisfied", function () {
        ...
        expect(RuleMatcher.matchPageUrlFilter(pageUrlFilter, requestDetails)).toBeFalsy();
      });
    });
    ...
    describe("#matchRequestWithRuleSourceFilters", function () {
      it("should return true only if all source filters are satisfied", function () {
        ...
        spyOn(window.tabService, "getTabUrl").andReturn(URL_SOURCES.EXAMPLE);
        expect(RuleMatcher.matchRequestWithRuleSourceFilters(sourceFilters, requestDetails)).toBeTruthy();
      });
      ...
      it("should return false if pageUrl filter is not satisfied", function () {
        ...
        spyOn(window.tabService, "getTabUrl").andReturn(URL_SOURCES.GOOGLE);
        expect(RuleMatcher.matchRequestWithRuleSourceFilters(sourceFilters, requestDetails)).toBeFalsy();
      });
      ...
      it("RQLY-61: should handle pageUrl has object has value instead of array", () => {
        ...
        spyOn(window.tabService, "getTabUrl").andReturn(URL_SOURCES.EXAMPLE);
        expect(RuleMatcher.matchRequestWithRuleSourceFilters(allFilters, requestDetails)).toBeTruthy();
      });
      ...
      it("should return false if sourceFilters are passed as array and all are not satisfied", () => {
        ...
        spyOn(window.tabService, "getTabUrl").andReturn(URL_SOURCES.EXAMPLE);
        expect(RuleMatcher.matchRequestWithRuleSourceFilters(allFilters, requestDetails)).toBeFalsy();
      });
